...
function buildClinicalPrompt(type: 'BLS' | 'ALS'): string {
  ...
  return (
    `אתה מדריך פרמדיק בכיר ישראלי ומומחה חינוך רפואי. משימתך ליצור שאלות קליניות מאתגרות לרמת ${type} בעברית רפואית מקצועית.\n\n` +
    `חומר מקור: בסס את כל השאלות והתשובות אך ורק על פרוטוקולים של משרד הבריאות הישראלי ומגן דוד אדום (מד"א). אל תשתמש בפרוטוקולים אמריקאיים (AHA/PHTLS) או אירופאיים אם הם סותרים את הסטנדרט הישראלי.\n\n` +
    `שפה: השתמש בעברית רפואית גבוהה, מקצועית וטבעית. הימנע מתרגומים מילוליים מאנגלית.\n\n` +
    ...
    'THE TRAP — DISTRACTOR RULES: At least TWO of the wrong options must be COMMON MISTAKES or OUTDATED PROTOCOLS. One wrong option must be a textbook-sounding answer that IGNORES the subtle twist in the scenario. Exactly ONE option follows current Israeli Ministry of Health / Israeli MDA protocol precisely. All distractors must be plausible to a junior provider — zero obviously wrong answers.\n\n' +
    'CLINICAL EXPLANATION: Exactly 3-4 concise Hebrew sentences, total under 60 words. Begin the explanation with the phrase: "על פי הפרוטוקולים בישראל..." (1) Explain the physiological mechanism making the correct action superior here. (2) Dissect the most tempting distractor and the SPECIFIC harm it causes. (3) End with one pearl that separates elite providers from average ones.\n\n' +
    'ACCURACY: Every drug dose, joule setting, rate threshold, and time window must match current Israeli Ministry of Health and Israeli MDA protocols EXACTLY. No approximations.\n\n' +
