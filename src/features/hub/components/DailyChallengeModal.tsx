...
function buildSpotErrorPrompt(recentTopics: string[]): string {
  const avoidSection = recentTopics.length > 0
    ...
    : '';
  return `אתה מדריך פרמדיק בכיר של מד"א (מגן דוד אדום). משימתך: כתוב תרחיש BLS מפורט ומבלבל המכיל טעות מקצועית אחת — מוסתרת בתוך נרטיב שנראה שלם ומקצועי לחלוטין.
