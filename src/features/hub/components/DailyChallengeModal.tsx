...
function buildImprovisedPrompt(recentTopics: string[]): string {
  const avoidSection = recentTopics.length > 0
    ...
    : '';
  return `אתה מדריך חובשים ישראלי. משימתך: צור שאלת "חובש ללא ציוד" — תרחיש חירום אמיתי בחיי היומיום, ללא תיק רפואי. החובש צריך לחשוב יצירתית ולהשתמש בחפצים ביתיים ופתרונות מאולתרים.

כללים מחייבים:
1. הגדרת מקום: ציין היכן המתנדב נמצא (פיקניק, קניון, אוטובוס, חתונה, מטיול, ביתו של שכן, משרד, חוף ים, אירוע ספורטיבי וכו') — ידגיש: אין תיק רפואי.
2. תרחיש (2-3 משפטים): תיאור ספציפי — גיל, מין, מנגנון/תסמינים. הדגש את חסרון הציוד.
3. שאלה: "מהו הפתרון המאולתר הטוב ביותר שניתן למצוא כאן?"
4. 4 תשובות: חפצים ביתיים נפוצים או פעולות — אחת נכונה, שלוש מסיחות הגיוניות. ערבב מיקום התשובה.
5. הסבר (2-3 משפטים): מדוע זהו הפתרון הטוב ביותר, כיצד הוא עוזר בפועל.
6. topic_tag: נושא קצר (1-3 מילים).

סבב בין: חנק, דימום פתוח, כוויה, אובדן הכרה, כאב לב, נפילה/שבר, אנפילקסיס, חשיפה לחום קיצוני, עקיצת דבורה/צרעה.
${avoidSection}
פלט JSON תקני בלבד, ללא markdown:
{
  "scenario": "תיאור התרחיש (2-3 משפטים — גיל, מין, מנגנון, מקום ללא ציוד)",
