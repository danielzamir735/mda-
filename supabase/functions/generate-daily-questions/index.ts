...
async function generateClinical(type: 'BLS' | 'ALS'): Promise<ClinicalQuestion> {
  const q = await withRetry(() => parseGeminiJSON<ClinicalQuestion>(buildClinicalPrompt(type)))
  if (
...
Deno.serve(async (req) => {
  ...
  const categories: Array<{ type: string; generate: () => Promise<unknown> }> = [
    { type: 'bls', generate: () => generateClinical('BLS') },
    { type: 'als', generate: () => generateClinical('ALS') },
    { type: 'med_v4', generate: () => generateMed(today) },
    { type: 'abbr', generate: () => generateAbbreviation() },
